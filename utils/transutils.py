...
def loadpic(url,binary=False,binaryReverse=False,binaryThresholdFactor=0.5,canny=False):
    img=cv2.imread(url)
    img=cv2.resize(img,(224,224))
    if binary:
        img=cv2.threshold(img,np.max(img)*binaryThresholdFactor,1,cv2.THRESH_BINARY)[1]
    if binaryReverse:
        img=cv2.threshold(img,np.max(img)*binaryThresholdFactor,1,cv2.THRESH_BINARY_INV)[1]
    if canny:
